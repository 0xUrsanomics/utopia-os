...
**Verification helper**: `scripts/eval/verify_subagent_diff.py`. takes claimed `files_changed` list + dispatch timestamp, reports verified / missing-claimed / unexpected-modifications. Run before reporting task completion to user.
...
#    scripts/eval/verify_subagent_diff.py uses time-based scope (--since + --claimed). Combine with
#    --scope pointing at the worktree dir for containment-aware checks.
DISPATCH_START_ISO="<ISO timestamp captured before step 1>"
python3 scripts/eval/verify_subagent_diff.py \
...
- **Verification loop**: pairs cleanly with `scripts/eval/verify_subagent_diff.py`. verify before merge, not after.
