...
- `task_config.prompt`: "Run skills/reality-review-weekly.md. Surface due entries to the inbox channel."

**Manual invocation**: operator can run `python3 scripts/reality_review_weekly.py --digest` anytime to see the pending review queue.

## Weekly flow

1. **Cron fires** weekly
2. **Skill runs** `reality_review_weekly.py --digest`
...
python3 scripts/reality_review_weekly.py --digest

# Show stats / labeling rate
python3 scripts/reality_review_weekly.py --stats

# Mark an entry
python3 scripts/reality_review_weekly.py --mark 42 positive --note "goal met"

# JSON for scripting
python3 scripts/reality_review_weekly.py --json
```

## Pairs with

- **Auto-create** (`skills/reality-feedback-create.md`): writes the entries that this skill reviews.
- **Layer 1 classifier** (`stake_classifier.py`): outcomes feed v2 classifier training data over months.
