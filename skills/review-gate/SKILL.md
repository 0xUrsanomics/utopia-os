...
Apply the `anti-ai-slop` skill's Mode 2 (explicit review) to the content body. NOT the frontmatter. Collect any rule violations (em dash abuse, inflated symbolism, promotional tone, hedging, AI verb crutches, etc.. full ruleset in `skills-shared/anti-ai-slop/SKILL.md`).
...
If the raw item is an HTML doc meant to render to PDF (a one-pager, letter, long-doc, portfolio, resume), and you have a doc-typeset/PDF pipeline wired up (NOT bundled in this skeleton, bring your own), run its quality gate BEFORE approval. Illustrative shape:

1. **Placeholder check** (must pass): `python <your-doc-typeset>/build.py --check-placeholders path/to/doc.html` → must return "0 unfilled placeholders". If it fails, flag don't approve.
2. **Render to PDF** (once placeholders are clean): `python <your-doc-typeset>/build.py --render <template-name> --input path/to/doc.html --output outputs/drafts/path/to/doc.pdf`
3. **Verify page count + font embedding**: `python <your-doc-typeset>/build.py --verify <template-name>` → must PASS (no issues listed).
