...
Before the audit, run `python3 scripts/check_standdown.py "<skill-name-or-source-repo>"`. If the registry already has a `block` or `expired` verdict, surface that and exit. No need to re-derive a known reject.
...
Per arxiv 2605.00055 (Ambient Persuasion), referenced in the CLAUDE.md security-first rules. The auto-compound counter at `scripts/auto_compound_counter.py` is one operational defense against this class.
...
audit_engine: skills/skill-audit.md
