...
5. **Post-save**: run `python3 scripts/memory/update_memory_active_goals.py` to refresh the MEMORY.md block.
...
```bash
python3 scripts/memory/update_memory_active_goals.py
```
...
`skills/save/SKILL.md` Step 3.7 runs `scripts/memory/goal_touch_session.py` which scans outputs/raw/**/*.md (last 24h) for frontmatter `goal: g-XXX` tags and bumps `sessions_touched` + `last_advanced` for matching active goals.

## Drift cron (v2 extends)

`scripts/memory/goal_drift_audit.py` (weekly). Findings:
...
- Existing patterns lifted: the stand-down registry shape, the `state/` convention, the `skills/save/SKILL.md` extraction discipline.
