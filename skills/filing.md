...
```bash
python3 scripts/file_intake.py --verbose
```
...
```bash
python3 scripts/file_intake.py --apply           # COPY into archive/ + index
python3 scripts/file_intake.py --apply --drain   # MOVE (inbox becomes staging)
```
...
`scripts/file_intake.py`. Order matters, first match wins, so put specific patterns
