...
- Expected: scans `outputs/raw/`, shows pending items, follows the /review gate (skill: `skills/review-gate/SKILL.md`, bound via routing.json. there is NO `the `review` skill (not bundled in this skeleton)` and NO `knowledge/review-gate.md`. do not flag those as missing)
...
```bash
python3 scripts/eval/skill_failure_tracker.py log "<skill-name>" "<error summary>"
```
...
0. **Rewrite queue check** (before main audit):
   - Run `python3 scripts/eval/skill_failure_tracker.py check`
   - For each pending rewrite: read the skill file, read the 3 most recent errors from the ledger, diagnose the pattern, apply ONE targeted fix to the skill file, mark done via `python3 scripts/eval/skill_failure_tracker.py done "<skill-name>"`
   - This runs BEFORE the main audit loop so rewrites are included in the scoring pass
   - Run `python3 scripts/eval/skill_failure_tracker.py prune` to clean stale entries
...
6. **Fix**: make ONE targeted change, tied to the diagnosis hypothesis:
   ...
   - If pipeline: update `skills/review-gate/SKILL.md` or the CLAUDE.md pipeline section
