...
trigger: premortem this, premortem my, run a premortem, what could kill this, future-proof this, stress test this plan, find the blind spots, poke holes in this, where will this break, devil's advocate this
---
...
- `CLAUDE.md` (business context, preferences, constraints)
...
Per Delegation discipline (CLAUDE.md): max-depth 2, no recursion. Each agent dispatched here is depth-1. Brief includes explicit "no em-dashes, no AI slop wordlist" output constraint to keep prose clean.
...
Output constraints: no em-dashes (use periods or hyphens), no AI slop wordlist (leverage / delve / navigate / holistic / seamless / underscore / tapestry / testament to), no inflated symbolism, no AI-cliche openers, no formulaic transitions.
...
**File:** `outputs/raw/agent/premortem-report-[timestamp].html`
...
- Round-robin visual: show the number of agents that ran and their findings as a grid or card layout, so the user can see the full scope of the premortem at a glance
- Footer with timestamp and what was premortemed

Run the anti-ai-slop chain on the HTML body before saving (per render-skill chain pattern). Em-dash sweep mandatory.

### step 6: save the transcript

Save the full premortem transcript as `outputs/raw/agent/premortem-transcript-[timestamp].md` in the same location. This includes:
...
```
outputs/raw/agent/premortem-report-[timestamp].html    # visual report for scanning
outputs/raw/agent/premortem-transcript-[timestamp].md  # full transcript for reference
```
...
**User:** "premortem this: I'm about to launch a $297 live workshop on how to use an AI tool for marketing teams. 50 seats. Targeting marketing managers at companies with 10-50 employees."
...
2. "AI tool for marketing" is a tool-specific pitch in a market where most managers are still figuring out whether AI is relevant to them at all
...
## relationship to decision-council

| If user wants | Use |
|---|---|
| "5 perspectives on whether this is a good idea" | decision-council |
| "every reason this could fail before I commit" | premortem (this skill) |
| "stress test from multiple angles" | decision-council |
| "what am I missing / blind spots" | premortem |
| "argue both sides" | decision-council |
| "future-proof this" | premortem |

If the request is ambiguous, ask once: "do you want multiple perspectives debating now, or future-failure analysis?" Don't run both unless the user explicitly asks for both.

## provenance

Lifted from a shared doc. Source: Klein method per HBR. Em-dash sweep applied + AI-slop output constraint added to the sub-agent brief template + workspace paths adjusted to outputs/raw/agent/ for the pipeline + a reciprocal pointer to decision-council.md added.
