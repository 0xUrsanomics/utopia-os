...
metadata:
  ...
  category: marketing
  updated: 2026-03-06
audit_exempt: external-lift-mit
audit_exempt_reason: MIT lift from Alireza Rezvani's marketing skill collection. Modifications technically permitted but break upstream-diff fidelity. Exempted from the 9-element quality gate as an external MIT lift.
---

# AI SEO

Goal: get content extracted, quoted, cited by ChatGPT, Perplexity, Google AI Overviews, Claude, Gemini, Copilot.

Not traditional SEO. Traditional SEO = ranked. AI SEO = cited. Different games, different rules.

**Voice when reporting back:** blunt, short fragments, no hedging, no "it's worth considering." State what's broken, what to do, in that order. Skip throat-clearing.

## Quick Start (5 minutes)

No audit yet? Run this first:
1. `curl yourdomain.com/robots.txt`: verify GPTBot, PerplexityBot, ClaudeBot, Google-Extended NOT blocked. If blocked → fix first, everything else waits.
2. Pick 3 target queries. Test each on perplexity.ai. Log: cited yes/no, who got cited, what format (def / list / table).
3. Pick the winning format from step 2. That's the template to copy on your pages.

Then proceed to full audit below.
...
Output rules:
- Bottom line first. Answer, then why.
- Every finding: what + why + how to fix.
- Actions name an owner + date. No "consider reviewing."
- Confidence tag on every claim: 🟢 tested / 🟡 pattern / 🔴 guess.
- Cut filler. "Significant improvement" → "40% faster." "Studies show" → "[Source, Year]."

Field is young. Tag every claim: proven (citation test) vs pattern-match vs guess. Don't hedge. label.
