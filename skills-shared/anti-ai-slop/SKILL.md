...
For VISUAL deliverables (decks, HTML, sponsor PDFs, landing pages), pair this skill with `the `anti-ai-design` skill (not bundled in this skeleton)`. That sibling catches typography / palette / layout / numbered-grid / equal-bullet-parallelism visual tells that this skill (copy-only) cannot see. Both gates fire together as pre-publish checks for any external-facing visual deliverable. Standard chain order:

```
deck-design → pitch-deck-builder → anti-ai-slop (this skill, copy) → anti-ai-design (visual) → html-to-pdf-deck
```

See `the `deck-design` skill (not bundled in this skeleton)` Integration section + `the `anti-ai-design` skill (not bundled in this skeleton)` for the full audit checklist.
