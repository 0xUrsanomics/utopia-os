...
- **Use a clean browser profile.** Polymarket may trigger the wallet-connect modal if a previous session left a wallet linked. A profile with no wallet linked is the reliable default for read-only scraping.
